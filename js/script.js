...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function resetPanelAnimations(panel) {
    ...
    // (The #skills panel manages its own fills via the slide/video controller below.)
    if (panel.id !== 'skills') {
  ...
  /* ---------------- Skills panel: slide-in → dwell → slide-out → video → loop ----------------
     The skill list slides in from the left, stays on screen for 7s (holding the
     pointer down pauses that countdown), then slides out to the right. The
     background video only plays while the list is off-screen; once the video
     finishes, the list resets to the left and slides in again — looping for as
     long as the Skills panel stays active. */
  (function initSkillsPanel() {
    const panel = document.getElementById('skills');
    const stage = panel.querySelector('.skills-stage');
    const slide = panel.querySelector('.skills-slide');
    const video = panel.querySelector('.skills-bg-video');
    if (!stage || !slide || !video) return;

    const DWELL_MS = 7000;
    const SLIDE_MS = 900; // must match the .skills-slide transition duration in CSS
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    let active = false;
    let phase = 'idle'; // idle | in | out
    let paused = false;
    let dwellStart = 0;
    ...
    function setFills(on) {
      slide.querySelectorAll('.skill-row').forEach(row => {
        const fill = row.querySelector('.skill-fill');
        if (!fill) return;
        if (!on) {
          fill.style.transition = 'none';
    ...
    function goSlideIn() {
      ...
      // Snap to the left, off-screen, with no transition, then animate in.
      slide.classList.add('pos-instant');
      slide.classList.remove('is-out');
      slide.classList.remove('is-in');
      setFills(false);
      forceReflow(slide);
      slide.classList.remove('pos-instant');
    ...
    function startDwell() {
      ...
      if (reduceMotion) return; // reduced motion: stay put, skip the loop entirely
      dwellTimer = setTimeout(onDwellEnd, dwellRemaining);
    }
    ...
    function goSlideOut() {
      ...
      slideOutTimer = setTimeout(() => {
        if (!active) return;
        playVideo(); // background video plays only during this off-screen phase
      }, SLIDE_MS);
    }

    function pauseDwell() {
      if (phase !== 'in' || paused || reduceMotion) return;
      paused = true;
      clearTimeout(dwellTimer);
      dwellRemaining -= (performance.now() - dwellStart);
    ...
    video.addEventListener('ended', () => {
      if (!active) return;
      goSlideIn(); // loop: back to the start once the video has fully played
    });

    // Press-and-hold (mouse or touch) pauses the 7s dwell; releasing resumes it.
    stage.addEventListener('pointerdown', pauseDwell);
    stage.addEventListener('pointerup', resumeDwell);
    stage.addEventListener('pointerleave', resumeDwell);
    stage.addEventListener('pointercancel', resumeDwell);
    ...
    window.skillsPanel = {
      ...
      stop() {
        ...
        clearTimeout(slideOutTimer);
        video.pause();
        slide.classList.remove('is-in', 'is-out');
        setFills(false);
      }
    };
  })();
