...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Every time a panel is (re)visited, its reveal animations, skill bars,
  // and counters are reset and replayed from scratch — not just on first view.
  function activatePanelContent(panel) {
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    /* ---- Reveal / fade-up elements ---- */
    const revealEls = panel.querySelectorAll('.reveal');
    revealEls.forEach(el => el.classList.remove('in-view'));

    /* ---- Skill bars ---- */
    const skillFills = panel.querySelectorAll('.skill-row .skill-fill');
    skillFills.forEach(fill => { fill.style.transition = 'none'; fill.style.width = '0%'; });

    /* ---- Counters: stop any in-flight run and reset to 0 ---- */
    const counters = panel.querySelectorAll('.num[data-count-to]');
    counters.forEach(el => {
    ...
    // Force a reflow so the browser registers the "reset" state above before
    // we flip classes/styles back on — otherwise CSS transitions won't replay.
    ...
    requestAnimationFrame(() => {
      ...
      counters.forEach(el => {
        ...
        // Slow, deliberate count-up: each number is held on screen for .75s
        // before advancing to the next, right up until the target is reached.
        let current = 0;
  ...
  /* ---------------- Glitch effect on name ----------------
     Plays once per page load only — intentionally excluded from the panel
     re-trigger behavior above — but runs for a longer, 4.7s overall duration. */
  const nameEl = document.querySelector('.name-glitch');
  if (nameEl) {
    requestAnimationFrame(() => nameEl.classList.add('glitching'));
    setTimeout(() => nameEl.classList.remove('glitching'), 4700);
  }
